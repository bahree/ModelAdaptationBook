...
Reference selection (--reference):
    human   (default) score every model against the ORIGINAL human answer
            for each prompt (from the shared IT-support source data). This
            is the methodologically correct choice for measuring
            distillation retention: the reference is independent of all
            three models, so you get the expected base < student < teacher
            ordering and a meaningful retention percentage.
...
def load_human_references(source_dir: str) -> dict:
    """Build a prompt -> original human answer map from the shared
    IT-support source data (train + valid). The teacher prompts were drawn
    from this set, so every distillation prompt has a corresponding human
    reference."""
    refs = {}
    for fn in ("train.jsonl", "valid.jsonl"):
        path = Path(source_dir) / fn
        if not path.exists():
            continue
...
def evaluate_model(model, tokenizer, examples, label: str,
    ...
    reference_mode="human": score against the original human answer from
    the IT-support source (looked up by prompt). reference_mode="teacher":
    score against the
...
def main():
    ...
    parser.add_argument(
        "--teacher_dir", required=True,
        help="Teacher model: accepts a local path, an HF repo id, or "
             "'repo#subfolder' (e.g. bahree/ModelAdaptationBook#ch6-sft)")
    parser.add_argument(
        "--student_dir", required=True,
        help="Student adapter: accepts a local path, an HF repo id, or "
    ...
    parser.add_argument(
        ...
        help="Base model: accepts a local path, an HF repo id, or "
             "'repo#subfolder' (e.g. bahree/ModelAdaptationBook#ch6-sft)")
    parser.add_argument("--source_dir", default="data/it_support_fmt",
    ...
    parser.add_argument("--eval_file", default=None,
                        help="Evaluate on this messages JSONL instead of <data_dir>/valid.jsonl (e.g. data/it_support/test.jsonl, "
                             "the held-out split; its own assistant turns are used as the human references)")
    args = parser.parse_args()

    seed_everything(42)

    valid_path = Path(args.eval_file) if args.eval_file else Path(args.data_dir) / "valid.jsonl"
    examples = list(read_jsonl(str(valid_path)))
    print(f"Loaded {len(examples)} test examples from {valid_path}")

    human_refs = {}
    if args.reference == "human":
        human_refs = load_human_references(args.source_dir)
        if args.eval_file:
            for ex in examples:
                msgs = ex.get("messages", [])
                user = next((m["content"] for m in msgs if m["role"] == "user"), None)
                asst = next((m["content"] for m in msgs if m["role"] == "assistant"), None)
                if user is not None and asst is not None:
                    human_refs.setdefault(user, asst)
        covered = sum(
        ...
        print(f"Reference mode: HUMAN ({covered}/{len(examples)} prompts matched "
              f"to human answers in {args.source_dir})")
    else:
    ...
    tokenizer = AutoTokenizer.from_pretrained(base_p, **subfolder_kwargs(base_s), trust_remote_code=True)
    model = AutoModelForCausalLM.from_pretrained(
        base_p, **subfolder_kwargs(base_s), torch_dtype=torch.bfloat16, device_map="auto",
    ...
    teacher_p, teacher_s = split_ref(args.teacher_dir)
    tokenizer = AutoTokenizer.from_pretrained(teacher_p, **subfolder_kwargs(teacher_s), trust_remote_code=True)
    model = AutoModelForCausalLM.from_pretrained(
        teacher_p, **subfolder_kwargs(teacher_s), torch_dtype=torch.bfloat16, device_map="auto",
        trust_remote_code=True,
    )
    model.eval()
    ...
    student_p, student_s = split_ref(args.student_dir)
    tokenizer = AutoTokenizer.from_pretrained(student_p, **subfolder_kwargs(student_s), trust_remote_code=True)
    base_p, base_s = split_ref(args.base_model)
    model = AutoModelForCausalLM.from_pretrained(
        base_p, **subfolder_kwargs(base_s), torch_dtype=torch.bfloat16, device_map="auto",
        trust_remote_code=True,
    )
    model = PeftModel.from_pretrained(model, student_p, **subfolder_kwargs(student_s))
    model.eval()
