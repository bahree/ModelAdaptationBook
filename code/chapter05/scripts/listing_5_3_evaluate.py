...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--test",
        "--dolly_test",  # older name, kept so existing commands keep working
        dest="test",
        default=None,
        help="Held-out test set JSONL for the primary evaluation (data/it_support/test.jsonl; no training or model-selection step touches it)",
    )
    ...
    ap.add_argument("--render_only", action="store_true", help="Re-render report.md from an existing report.json in --out (no model evaluation)")
...
def summarize_variant(name: str, model, tokenizer, args: argparse.Namespace) -> Dict[str, Any]:
    ...
    Evaluates on the held-out test set (instruction-following), the toy golden
    ...
    Returns:
        Dict with evaluation results keyed by test type ("test", "toy", "safety").
    """
    result: Dict[str, Any] = {"name": name}
    
    # Primary evaluation: held-out test set
    if args.test and Path(args.test).exists():
        test_result = eval_test_set(
            model,
            tokenizer,
...
def render_markdown(res: Dict[str, Any], args: argparse.Namespace) -> str:
    """Render the human-readable report.md from the results dict (also used by --render_only)."""
    ...
    if args.test:
        lines.append(f"- Test set: `{args.test}`")
    if args.adapter:
    ...
    for key in ["base", "adapter", "adapter_alt"]:
        ...
        if variant.get("test"):
            d = variant["test"]
            lines.append("### Test Set (Instruction-Following)")
    ...
    def add_delta_block(delta_key: str, label: str) -> None:
        ...
        # Primary: test-set metrics
        if d.get("test"):
            lines.append("### Test Set Improvements")
            lines.append(f"- **Overall exact match Δ**: {fmt_delta(d['test']['exact_match'], pct=True)}")
            lines.append(f"- **Overall token-F1 Δ**: {fmt_delta(d['test']['token_f1'])}")
            if d["test"].get("category_metrics"):
                lines.append("\n**Per-Category Improvements:**")
                for cat, metrics in sorted(d["test"]["category_metrics"].items()):
...
def main() -> None:
    ...
    if args.render_only:
        res = json.loads((out_dir / "report.json").read_text(encoding="utf-8"))
        (out_dir / "report.md").write_text(render_markdown(res, args), encoding="utf-8")
        console.print(f"[green]✓[/green] Re-rendered {out_dir / 'report.md'} from report.json")
        return
    ...
    def compute_deltas(base: Dict[str, Any], other: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if base.get("test") and other.get("test"):
            deltas["test"] = {
                "exact_match": maybe_delta(other["test"]["exact_match"], base["test"]["exact_match"]),
                "token_f1": maybe_delta(other["test"]["token_f1"], base["test"]["token_f1"]),
                "category_metrics": {},
            }
            # Per-category deltas
            base_cats = base["test"].get("category_metrics", {})
            other_cats = other["test"].get("category_metrics", {})
            for cat in set(base_cats.keys()) | set(other_cats.keys()):
                if cat in base_cats and cat in other_cats:
                    deltas["test"]["category_metrics"][cat] = {
